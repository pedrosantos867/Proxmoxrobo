                    ...
                    <th style="width: 15%"><?= $_->l('Услуга') ?>
                        ...
                        <div>
                            <select name="type" class="filter" data-type="equal">
                                ...
                                <option
                                    <?= (isset($filter['type']) && $filter['type'] != '' && $filter['type'] == \model\Bill::TYPE_ORDER) ? 'selected="selected"' : '' ?>
                                    value="<?= \model\Bill::TYPE_ORDER ?>"><?=$_->l('Хостинг')?>
                                </option>
                                <option
                                    <?= (isset($filter['type']) && $filter['type'] == \model\Bill::TYPE_CHANGE_PLAN) ? 'selected="selected"' : '' ?>
                                    value="<?= \model\Bill::TYPE_CHANGE_PLAN ?>"><?=$_->l('Смена тарифа')?>
                                </option>

                                <option
                                    <?= (isset($filter['type']) && $filter['type'] == \model\Bill::TYPE_BALANCE) ? 'selected="selected"' : '' ?>
                                    value="<?= \model\Bill::TYPE_BALANCE ?>"><?=$_->l('Пополнение баланса')?>
                                </option>

                                <?if($config->enable_component_vps){?>
                                <option
                                    <?= (isset($filter['type']) && $filter['type'] == \model\Bill::TYPE_VPS) ? 'selected="selected"' : '' ?>
                                ...
                                <option
                                    <?= (isset($filter['type']) && $filter['type'] == \model\Bill::TYPE_DOMAIN_ORDER) ? 'selected="selected"' : '' ?>
                                    value="<?= \model\Bill::TYPE_DOMAIN_ORDER ?>"><?= $_->l('Домен') ?>
                                </option>
                                <option
                                    <?= (isset($filter['type']) && $filter['type'] == \model\Bill::TYPE_DOMAIN_PROLONG) ? 'selected="selected"' : '' ?>
                                ...
                                <option
                                    <?= (isset($filter['type']) && $filter['type'] == 's'.$category->id.'') ? 'selected="selected"' : '' ?>
                                    value="s<?=$category->id?>"><?=$category->name?> </option>
                                <?}?>
                                <? foreach ($services as $key=>$service) {?>
                                <option
                                    <?= (isset($filter['type']) && $filter['type'] == $key) ? 'selected="selected"' : '' ?>
                    ...
                    <th style="width: 10%"><?= $_->l('Статус') ?>
                        ...
                        <div>
                            <select name="is_paid" class="filter" data-type="equal">
                                <option value=""> ---</option>
                                <option
                                    <?= (isset($filter['is_paid']) && $filter['is_paid'] != '' && $filter['is_paid'] == 0) ? 'selected="selected"' : '' ?>
                                    value="0"><?= $_->l('Ожидает оплату') ?>
                                </option>
                                <option
                                    <?= (isset($filter['is_paid']) && $filter['is_paid'] == -1) ? 'selected="selected"' : '' ?>
                                    value="-1"><?= $_->l('Отменен') ?>
                                </option>
                                <option
                                    <?= (isset($filter['is_paid']) && $filter['is_paid'] == 1) ? 'selected="selected"' : '' ?>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <?=$_->l('VPS')?>
                        <? } elseif ($bill->type == \model\Bill::TYPE_BACKUP) { ?>
                        <?=$_->l('Backup')?>
                        <? } else { ?>
                    ...
                    <td class="text-center">
                        ...
                        <div class="btn-group-vertical" role="group" aria-label="...">
                            <a href="<?= $_->link('bill/' . $bill->id) ?>" class="btn btn-primary btn-xs"><span
