<div class="ajax-block">
    <a href="<?= $_->link('admin/server/add') ?>" class="btn btn-default ajax-modal"><span
    ...
    <table class="table table-bordered">
        ...
        <? foreach ($servers as $server) { ?>
            <tr <?=$server->hidden ? 'class="row_hidden"' : ''?> data-id="<?=$server->id?>">
                ...
                <td><?= $server->name ?></td>
                <td><?= $server->host ?></td>
                <td><a class="btn btn-warning btn-xs ajax-action" href="<?= $_->link('admin/server/check/' . $server->id) ?>">
                        <span class="glyphicon glyphicon-transfer"></span> &nbsp;
                        <?=$_->l('Проверить соединение')?></a></td>
                <td>
                    <a class="btn btn-default btn-xs ajax-modal"
                       href="<?= $_->link('admin/server/' . $server->id) ?>"><span class="glyphicon glyphicon-cog"
                                                                                   aria-hidden="true"></span><?=$_->l('Редактировать')?></a>
                    <a class="btn btn-danger btn-xs ajax-action"
                       href="<?= $_->link('admin/server/remove/' . $server->id) ?>"
                    ...
                    <span
                        class="glyphicon glyphicon-trash" aria-hidden="true"></span><?=$_->l('Удалить')?>
                    </a>
                    <?if($server->hidden){?>
